...
class Rev:
    ...
    def check(self, x, y, fn, count, minute, f_name):
        ...
        for i in self.clist[:-1]:       # iterate through all other 19 previous centroids (not the closets previous centroid)
            ...
            if cur_d > prev_d:              # if the distance for last centroid is greater than any other previous centroid, than the new centroid has reversed!
                ...
                self.clist = []         # clear the centroid list so as to not recalculate and reversal again
                count = 0               # set count to 0 so we can add the next 20 centroids into our list
                self.rev_list.append(['R',minute // 120])
                self.save_to_file(f_name, minute)
    ...
    def save_to_file(self, f_name, minute):
        ...
        with open(path + 'rev_' + f_name + ".csv", 'a', newline="") as nf:   # open a csv file for corresponding new data set
            ...
            row = self.rev_list[-1] #create row for csv including minute number and the current count for reversals
