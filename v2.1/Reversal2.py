...
class Reversal2:
    ...
    def check_rev(self, x, y):
        ...
        self.dis = dist_from_cur_to_prev                                                    # set dis field to last distance from current to previous centroid
        self.change_x = float(x) - float(self.prev_centroid[-1][0])
        #old_change_x = float(self.prev_centroid[-1][0]) - float(self.prev_centroid[-2][0]) 
        self.change_y = float(y) - float(self.prev_centroid[-1][1])
        #old_chnage_y =float(self.prev_centroid[-1][1]) - float(self.prev_centroid[-2][1])
        ...
        for i in self.prev_centroid[-2::-1]:                                                # iterate through all ther previous centroids
            ...
            if dist_from_cur_to_prev > dist_from_cur_to_others + 7.75:           
                return True
    ...
    def save(self, file, frame_num, rev, minute_interval):
        ...
        with open(os.getcwd() + "\\full_data_2fps\\rev_" + file, 'a', newline="") as nf:    # open file in new folder to store reversal count in a given frame per minute interval
            ...
            row = [frame_num, rev, minute_interval, self.dis, self.change_x, self.change_y]                               # create row for csv including minute number and the current count for reversals
